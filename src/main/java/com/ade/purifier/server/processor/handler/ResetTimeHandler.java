 ...
 */
public class ResetTimeHandler implements BasicHandler {
    ...
    private final static String KEY_FILTERTIME     = "filterTime";
    private final static String KEY_FILTERLIFTTIME = "filterLiftTime";
    private final static String KEY_MACHINETIME    = "machineTime";
    ...
    public Map<String, Object> work(String mobile, Object obj, ObjectContext context) {
        ...
        if (obj instanceof Map<?, ?>) {
            ...
            if (acp == null || !acp.isConnected()) {
                ...
                byte[] instruction = MsgUtils.createAcpInstruction((byte) 0x92, (byte) set);
            ...
            logger.error("LockReq信息结构错误.");
            map.put(KEY_CODE, -1);
            map.put(KEY_MESSAGE, "LockReq信息结构错误.");
            return map;
        }
    }
}
