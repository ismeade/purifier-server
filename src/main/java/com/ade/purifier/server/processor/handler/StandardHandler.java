 ...
 */
public class StandardHandler implements BasicHandler {
    ...
    public Map<String, Object> work(String mobile, Object obj, ObjectContext context) {
        ...
        if (obj instanceof Map<?, ?>) {
            ...
            if (acp == null || !acp.isConnected()) {
                ...
                byte[] instruction = MsgUtils.createAcpInstruction((byte) 0xA6, (byte) set);
            ...
            logger.error("SpeedReq信息结构错误.");
            map.put(KEY_CODE, -1);
            map.put(KEY_MESSAGE, "SpeedReq信息结构错误.");
            return map;
        }
    }
