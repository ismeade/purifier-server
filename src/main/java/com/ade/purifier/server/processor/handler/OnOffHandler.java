 ...
 */
public class OnOffHandler implements BasicHandler {
    ...
    public Map<String, Object> work(String mobile, Object obj, ObjectContext context) {
        ...
        if (obj instanceof Map<?, ?>) {
            ...
            if (acp == null || !acp.isConnected()) {
                ...
                switch (set) {
                    case 1:
                        instruction = MsgUtils.createAcpInstruction((byte) 0x84, (byte) 0x01);
                        break;
                    case 0:
                        instruction = MsgUtils.createAcpInstruction((byte) 0x83, (byte) 0x01);
                        break;
                    default:
                        logger.error("无效的set值:" + set);
                        map.put(KEY_CODE, 2);
                        map.put(KEY_MESSAGE, "无效的set值:" + set);
                        return map;
                }
                // 与净化器通讯
            ...
            logger.error("onOff信息结构错误.");
            map.put(KEY_CODE, -1);
            map.put(KEY_MESSAGE, "onOff信息结构错误.");
            return map;
        }
    }
